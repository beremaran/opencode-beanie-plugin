...
test("updates progress, validates lifecycle changes, and supports replacement", async () => {
    ...
    const blockedCompletion = result(await update.execute({
        status: "completed",
        verificationEvidence: ["Not yet"]
    }, session)) as { error: { code: string; message: string } };
    ...
    const completedWithoutEvidence = result(await update.execute({status: "completed"}, session)) as {
    ...
    const terminal = result(await update.execute({progress: "late"}, session)) as {
        error: { code: string; message: string }
    };
