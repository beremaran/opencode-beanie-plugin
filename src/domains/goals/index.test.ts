...
test("recovers only the matching session and removes deleted goals", async () => {
  ...
  if (!compact) throw new Error("Goals compaction hook was not registered")
  ...
  if (!eventHook) throw new Error("Goals event hook was not registered")
...
test("dispose clears process state", async () => {
  ...
  if (!dispose) throw new Error("Goals dispose hook was not registered")
  await dispose()
