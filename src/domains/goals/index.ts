...
function canTransition(from: GoalStatus, to: GoalStatus) {
  ...
  if (from === "active") return ["active", "paused", "blocked", "completed", "cancelled"].includes(to)
  if (from === "paused") return ["paused", "active", "cancelled"].includes(to)
...
function updateGoal(goal: Goal, args: UpdateArgs) {
  ...
  if (!canTransition(goal.status, status)) return error(`Cannot transition ${goal.status} to ${status}.`)
...
export const GoalsDomain: Domain = () => {
  ...
  const goalUpdate = tool({
    ...
    execute(args, context) {
      ...
      if (!goal) return Promise.resolve(error("No goal exists for this session; use goal_set first."))
  ...
  return Promise.resolve({
    ...
    config: (config) => {
      ...
      if (!config.command?.goal) config.command = {
        ...config.command,
        goal: {
    ...
    event: ({ event }) => {
      if (event.type === "session.deleted") goals.delete(event.properties.info.id)
      return Promise.resolve()
    },
