...
import {extractDirs, fetchTree, resolveDir, splitSource, type TreeEntry} from "./github-tree";
import {collectMatches, type FrontmatterInfo, type SearchMatch, SEARCH_CANDIDATE_LIMIT} from "./github-search";
import {capSkillFiles} from "./github-files";
import {buildGithubSummary, loadDirFiles, toSummary} from "./github-loader";
...
export class GithubRegistry implements SkillRegistry {
  ...
  private async collectSourceSummaries(source: string, incDesc: boolean, out: SkillSummary[]) {
    ...
    for (const dir of extractDirs(tree.entries)) {
      out.push(await buildGithubSummary(parsed.owner, parsed.repo, tree.branch, dir, incDesc, this.frontmatter));
    }
  }
