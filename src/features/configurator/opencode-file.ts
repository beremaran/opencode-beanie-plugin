...
export function projectConfigPaths(worktree: string): string[] {
  return [join(worktree, "opencode.json"), join(worktree, "opencode.jsonc"), join(worktree, ".opencode", "opencode.json")]
}
...
export function resolveTargetPath(worktree: string, scope: ConfigScope = "auto"): string {
  const candidates = scope === "global" ? [globalConfigPath()] : scope === "project" ? projectConfigPaths(worktree) : candidateConfigPaths(worktree)
  for (const path of candidates) {
    const text = readConfigFile(path)
    if (text !== null && text.includes(PLUGIN_QUOTED)) return path
  }
...
export type ApplyResult = { path: string; created: boolean; changed: boolean }
