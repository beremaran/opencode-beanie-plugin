import { type Config, type Plugin, type PluginInput, tool } from '@opencode-ai/plugin'
import {
  parseBeanie,
  parseOptionsPayload,
  renderApply,
...
async function runConfigHook(client: PluginInput['client'], cfg: Config, state: BeanieState): Promise<void> {
  ...
  // biome-ignore lint/style/useNamingConvention: `subagent_depth` is opencode's own config field name (external JSON contract).
  const subagentDepth = (cfg as Config & { subagent_depth?: unknown }).subagent_depth
  let depth: number | undefined
  if (typeof subagentDepth === 'number') {
    depth = subagentDepth
  }
  ...
  await Promise.all(
    validation.errors.map((error) =>
      warn(client, `Existing configuration problem in "${error.feature}": ${error.message ?? ''}`),
    ),
  )
}
...
const configurePluginTool = (state: BeanieState, worktree: string) =>
  tool({
    ...
    // biome-ignore lint/suspicious/useAwait: the `tool` factory requires execute to return a Promise<ToolResult>.
    async execute(args, _context) {
      return executeConfigurePlugin(args, state, worktree)
    },
  })

// biome-ignore lint/suspicious/useAwait: the `Plugin` type requires the factory to return Promise<Hooks>.
const Configurator: Plugin = async ({ client, worktree }, _options = {}) => {
  const state: BeanieState = { options: {}, hasEntry: false }
  return {
    config: (cfg) => runConfigHook(client, cfg, state),
    // biome-ignore lint/suspicious/useAwait: the `command.execute.before` hook signature requires a Promise<void>.
    ...
    tool: {
      // biome-ignore lint/style/useNamingConvention: `configure_plugin` is the tool's public name (external agent tool API contract).
...
// biome-ignore lint/performance/noBarrelFile: intentional re-export surface for the plugin's shared configurator API.
