...
export function parseOptionsPayload(payload: string): { ok: true; options: Record<string, unknown> } | { ok: false; error: string } {
  const trimmed = payload.trim()
  if (trimmed === "") return { ok: true, options: {} }
  try {
    const parsed: unknown = JSON.parse(trimmed)
    if (typeof parsed !== "object" || parsed === null || Array.isArray(parsed)) return { ok: false, error: "The config must be a JSON object of feature options." }
...
export function renderValidation(result: ValidationResult): string {
  ...
  if (result.errors.length > 0) {
    lines.push("Errors:", ...result.errors.map((report) => `${report.ok ? "✓" : "✗"} ${report.feature}${report.message ? ` — ${report.message}` : ""}`), "")
  }
...
export function renderStatus(options: Record<string, unknown>, validation: ValidationResult, worktree: string, subagentDepth: number | undefined): string {
  ...
  if (subagentDepth !== undefined) {
    lines.push("", `Note: opencode \`subagent_depth\` is ${subagentDepth}; the orchestrator warns if \`orchestratorDepth\` exceeds it.`)
  }
  lines.push("", "Changes take effect after restarting opencode. Run /beanie init for a guided setup.")
  return lines.join("\n")
}

export function renderApply(options: Record<string, unknown>, result: { path: string; created: boolean }, validation: ValidationResult): string {
  const lines = [
