...
function schemaPayload(serverName: string, name: string, upstream: UpstreamTool) {
  return {
    ...
    ...(upstream.outputSchema === undefined ? {} : { outputSchema: JSON.parse(JSON.stringify(upstream.outputSchema)) }),
  }
}
    ...
    get_tool_schema: tool({
      ...
      async execute(args) {
        ...
        return JSON.stringify(
          upstream
            ? schemaPayload(target.server, target.tool, upstream)
            : { error: `unknown tool: ${target.server}__${target.tool}` },
          null,
          2,
        )
      },
