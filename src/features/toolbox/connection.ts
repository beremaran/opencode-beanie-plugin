...
type Session = { client: Client; transport: any; slot: boolean }
export class ConnectionManager {
  ...
  async getSession(name: string): Promise<Session> {
    ...
    if (this.closed) throw new Error('aggregator is shutting down')
  ...
  private async connect(name: string): Promise<Session> {
    ...
    try {
      ...
      transport.onclose = () => {
        if (!this.closed) this.teardown(name, null)
      }
      transport.onerror = (error: unknown) => {
        if (!this.closed) this.teardown(name, this.safe(error))
      }
  ...
  private transport(cfg: ServerConfig) {
    ...
    const url = new URL(cfg.url),
      options = { requestInit: { headers: { ...cfg.headers } } }
  ...
  async callTool(server: string, tool: string, args: Record<string, unknown>, signal?: AbortSignal) {
    const session = await this.getSession(server),
      result = await session.client.callTool({ name: tool, arguments: args }, {
        timeout: this.timeout(this.tools.upstream.get(server)!.config),
        ...(signal ? { signal } : {}),
      } as any)
    this.touch(server)
    return result
  ...
  private teardown(name: string, error: string | null) {
    ...
    if (session?.slot) this.release()
    if (session) void session.client.close().catch(() => undefined)
    if (error) this.tools.upstream.markError(name, error)
    else this.tools.upstream.setState(name, 'idle')
  }
  private acquire(timeout: number) {
    if (this.closed) return Promise.reject(new Error('shutdown'))
    if (this.available) {
      this.available--
  ...
  private safe(error: unknown) {
    ...
    if (typeof value.status === 'number') return `HTTP ${value.status}${value.statusText ? ` ${value.statusText}` : ''}`
