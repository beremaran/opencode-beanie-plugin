...
async function handleIdle(input: {
  ...
  try {
    ...
    if (decision.complete) {
      ...
      await showToast(input.client, `Goal complete: ${decision.reason}`, 'success')
      return
    }
  ...
  return {
    ...
    'command.execute.before': async (command, output) => {
      ...
      if (parsed.action === 'status') {
        ...
        replaceTextPart(output.parts, statusPrompt(await store.get(command.sessionID)))
        return
      }
      ...
      if (parsed.action === 'clear') {
        ...
        replaceTextPart(output.parts, actionPrompt('The session goal was cleared.'))
        return
      }
      ...
      if (parsed.action === 'pause') {
        ...
        if (current?.status !== 'active') {
          replaceTextPart(output.parts, actionPrompt('There is no active goal to pause.'))
          return
        }
        await store.set({ ...current, status: 'paused', updatedAt: Date.now(), lastReason: 'Paused by the user.' })
        replaceTextPart(output.parts, actionPrompt('The session goal is paused.'))
        return
      }
      if (parsed.action === 'resume') {
        if (!current) {
          replaceTextPart(output.parts, actionPrompt('There is no goal to resume.'))
          return
        }
        if (current.status === 'complete') {
          controlTurns.add(command.sessionID)
          replaceTextPart(output.parts, actionPrompt('The previous goal is complete. Set a new goal to do more work.'))
          return
        }
        const resumed = {
          ...current,
    ...
    event: async ({ event }) => {
      ...
      if (
        ...
        if (goal?.status === 'active') {
          ...
          await showToast(input.client, 'Goal paused after interruption', 'warning')
        }
        return
      }
